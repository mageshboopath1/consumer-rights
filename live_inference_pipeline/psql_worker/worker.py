    ...
    try:
        if op == "CREATE" and data:
            ...
            if not isinstance(condition, dict) or len(condition) != 1:
                print(
                    "ERROR: Condition must be a dict with exactly one key-value pair."
                )
                return False
            ...
            query = sql.SQL("DELETE FROM {} WHERE {} = {}").format(
                sql.Identifier(table), sql.Identifier(condition_col), sql.Placeholder()
            )
        ...
        print(
            f"DATABASE ERROR: Failed to execute operation. Rolling back. Error: {db_error}"
        )
        db_conn.rollback()
        return False
