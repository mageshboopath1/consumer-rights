...
def query_bedrock(prompt: str) -> str:
    ...
    try:
        ...
        print(f"[+] Received response from Bedrock in {elapsed_time:.2f}s ({len(output)} characters)")
        return output
        ...
        print(f"[!] Please check: 1) AWS credentials are correct, 2) IAM permissions for Bedrock, 3) Model access is granted", file=sys.stderr)
...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            print(f"[>] Sent status update to '{PROCESS_QUEUE}': LLM processing started")
