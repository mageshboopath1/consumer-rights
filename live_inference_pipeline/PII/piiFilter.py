...
try:
    ...
    class DummyLimiter:
        def is_allowed(self, ip): return True, 0, "OK"
        def check_burst_attack(self, ip): return False, "OK"
        def check_distributed_attack(self, ip): return False, "OK"
        def can_process_query(self): return True, "OK", {}
        def record_query(self): pass
...
def process_message(body, client_ip='127.0.0.1'):
    ...
    if SECURITY_ENABLED:
        ...
        if not can_process:
            ...
            print(f"    Daily: {cost_stats['daily']['cost']}/{cost_stats['daily']['budget']}")
            print(f"    Monthly: {cost_stats['monthly']['cost']}/{cost_stats['monthly']['budget']}")
        ...
        print(f"[i] Budget: Daily {cost_stats['daily']['percentage']}, Monthly {cost_stats['monthly']['percentage']}")
