...
class DDoSProtection:
    ...
    def check_burst_attack(self, ip_address: str) -> tuple[bool, str]:
        ...
        if len(self.burst_requests[identifier]) >= self.BURST_THRESHOLD:
            return True, f"BURST_ATTACK: {len(self.burst_requests[identifier])} requests in {self.BURST_WINDOW.seconds}s"
    ...
    def check_distributed_attack(self, ip_address: str) -> tuple[bool, str]:
        ...
        # Clean old requests
        self.distributed_requests = [
            (req_time, ip_hash) for req_time, ip_hash in self.distributed_requests
        ...
        if unique_ips >= self.DISTRIBUTED_THRESHOLD:
            if not self.under_attack:
                ...
                self.attack_count += 1
                return True, f"DDOS_ATTACK: {unique_ips} unique IPs in {self.DISTRIBUTED_WINDOW.seconds}s"
        else:
    ...
    def get_attack_stats(self) -> dict:
        ...
        return {
            ...
            'current_unique_ips': len(set(ip_hash for _, ip_hash in self.distributed_requests))
        }
