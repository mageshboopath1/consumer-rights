...
class RateLimiter:
    ...
    def _save_blocked_ips(self):
        ...
        try:
            data = {
                k: v.isoformat() 
                for k, v in self.blocked_ips.items()
            }
    ...
    def is_allowed(self, ip_address: str) -> tuple[bool, int, str]:
        ...
        if identifier in self.blocked_ips:
            ...
            if time_since_block < self.block_duration:
                ...
                remaining = int((self.block_duration - time_since_block).total_seconds())
        ...
        if current_count >= self.max_requests:
            ...
            if self.violation_count[identifier] >= self.block_threshold:
                ...
                self._save_blocked_ips()
                self._log_security_event('IP_BLOCKED', identifier, 
                                        f"Excessive violations: {self.violation_count[identifier]}")
                return False, int(self.block_duration.total_seconds()), "IP_BLOCKED_PERMANENT"
