...
class CostLimiter:
    def __init__(
        self, daily_budget_usd=0.50, monthly_budget_usd=5.00, cost_per_query=0.002
    ):
        """
        ...
        print(
            f"    Daily Budget: ${daily_budget_usd} ({self.daily_query_limit} queries)"
        )
        print(
            f"    Monthly Budget: ${monthly_budget_usd} ({self.monthly_query_limit} queries)"
        )
    ...
    def can_process_query(self) -> tuple[bool, str, dict]:
        ...
        if daily_count >= self.daily_query_limit:
            ...
            return (
                False,
                f"DAILY_BUDGET_EXCEEDED: ${daily_cost:.2f}/${self.daily_budget}",
                stats,
            )
        ...
        if monthly_count >= self.monthly_query_limit:
            ...
            return (
                False,
                f"MONTHLY_BUDGET_EXCEEDED: ${monthly_cost:.2f}/${self.monthly_budget}",
                stats,
            )
        ...
        if daily_count >= self.daily_query_limit * 0.8:
            print(
                f"[] WARNING: 80% of daily budget used ({daily_count}/{self.daily_query_limit} queries)"
            )

        if monthly_count >= self.monthly_query_limit * 0.8:
            print(
                f"[] WARNING: 80% of monthly budget used ({monthly_count}/{self.monthly_query_limit} queries)"
            )
    ...
    def _clean_old_data(self, now: datetime):
        ...
        self.daily_queries = [ts for ts in self.daily_queries if ts > day_ago]

        # Keep last 30 days for monthly
        month_ago = now - timedelta(days=30)
        self.monthly_queries = [ts for ts in self.monthly_queries if ts > month_ago]
...
cost_limiter = CostLimiter(
    daily_budget_usd=0.50, monthly_budget_usd=5.00, cost_per_query=0.002
)
