...
# This remains the same, connecting to the single RabbitMQ container.
connection = None
for i in range(10):
    try:
...
# --- Channel and Queue Setup ---
channel = connection.channel()

# Declare the queue we will READ from
channel.queue_declare(queue='terminal_messages', durable=True)

# --- RENAMED: Declare the queue we will WRITE to ---
...
def process_message(body):
    ...
    # --- UPDATED: Publish the result to the 'redacted_queue' ---
...
def callback(ch, method, properties, body):
    ...
    # Acknowledge the original message from 'terminal_messages'
